...
# a python script that extracts nucleotide diversity pi for RAD-tags using both .vcf genotypes AND .bam alignment files in order to cover RAD-tags that did not contain any variants and are thus missing from the .vcf; it applies missingness and quality filters to both input data
...
# usage example:
# python ./vcf_bam_unbiased_pi.py --bam_dir ../SNP_calling/ --sample_list males.txt --bam_suffix RG.bam --mindp 3 --maxdp 75 --max_missing 0.75 --vcf SNPs_mac1_maxmiss0.25.males_only.recode.vcf --target_contigs sex_spec
# Inputs
# Outputs	
